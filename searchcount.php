...
<table border='1'>
  <tr>
    ...
    <td>Количество данного СИ</td>
  </tr>

  <?php
    $sql  = mysqli_query($link, "SELECT COUNT( * ) AS `Строки` , `title_si`
FROM `Средства измерения`
GROUP BY `title_si` 
ORDER BY `title_si`
LIMIT 0 , 30") or die("Ошибка " . mysqli_error($link)); 
    while ($result = mysqli_fetch_array($sql)) {
      echo "<tr><td>{$result['title_si']}</td><td>{$result['Строки']}  </td></tr>";
    }
  ?>
</table>
</div>
