 ...
 <header> 
    ...
    <div class="info1">
        ...
        <form name="login" method="post">
            <p><input required name="login" type="text" size="30" placeholder="Логин"></p>
            <p><input required name="password" type="password" size="30" placeholder="Пароль"></p>
            <p><input required name="verification" type="password" size="30" placeholder="Повторите пароль"></p>
            <input type="submit" value="Создать аккаунт">
        </form>
        <a style="text-decoration: none; outline: none;" href="authorization.php">Авторизоваться</a>
        <?php
        if(isset($_POST["login"]) && isset($_POST["password"])&& isset($_POST["verification"])){
            $verification = $_POST["verification"];
            $login = $_POST["login"];
            $password = $_POST["password"];
            if($verification!=$password)
                echo "<p><font color='red'>Пароли не совпадают</font></p>";
            else {
                $user = mysqli_query($conn, "SELECT COUNT(*) FROM `Пользователи` WHERE `login`='$login'");
                $user = mysqli_fetch_all($user);
                if($user[0][0]>0)
                echo "<p><font color='red'>Пользователь с таким логином уже существует</font></p>";
                else {
                $new_user = mysqli_query($conn, "INSERT INTO `Пользователи`(`login`, `password`) VALUES ('$login','$password')");
                echo "<script>self.location='authorization.php';</script>";
                    }
            }
        }
        ?>
